...
export default function Checkout(props) {
  ...
  const [valuesatThesiErgasias, setvaluesatThesiErgasias] = React.useState([]);
...
const labeledQuestions = questions.map(q => ({
 ...
 React.useEffect(() => {
    ...
    fetch(`/query?thesi=${encodeURIComponent(thesiErgasias)}`)
      .then(res => res.json())
      .then(data => {
        // assuming data = [{ firstName, lastName }, …]
        setAutofillOptions(data.map(emp => emp.firstName));
      })
 ...
 const handleThesiChange = async (value)=> {
...
function getStepContent(step) {
  switch (step) {
    case 0:
      ...
      handleThesiChange={handleThesiChange}
 ...
 if (questions.length > 0) {
  ...
  const result = await submitForm();
     setSubmitFormSuccess(result);
  setActiveStep(3);   // Show confirmation or navigate if needed
    } catch (err) {
     console.error(err);
     setSubmitFormSuccess(false);   // mark failure
     setActiveStep(steps.length);   // still show confirmation UI
};
    }
  ...
  return (
    ...
    <Stack spacing={2} useFlexGap>
      ...
      <Button variant="contained" onClick={handleStart}>επιστροφη στην αρχικη</Button>
    </Stack>
  ) : (
    <Stack spacing={2} useFlexGap>
